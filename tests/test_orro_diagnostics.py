...
class OrroDiagnosticsTests(unittest.TestCase):
    def _init_home(self, root: Path) -> tuple[Path, Path]:
        ...
        with redirect_stdout(io.StringIO()):
            self.assertEqual(
                main(["init", "--home", str(home), "--repo", str(repo), "--depone-root", "/home/ubuntu/depone"]),
                0,
            )
        return repo, home
    ...
    def test_proofcheck_surfaces_untracked_source_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (evidence_dir / "team-ledger.json").write_text(
                json.dumps({"repo_root": str(repo), "lanes": [{"lane_id": "lane-a", "worktree": "worktrees/lane-a", "evidence_dir": "lane-a"}]}),
                encoding="utf-8",
            )
            ...
            with patch("witnessd.cli.verify._run_depone_json", side_effect=fake_depone):
                with redirect_stdout(stdout):
                    code = main(["proofcheck", str(evidence_dir), "--home", str(home), "--json"])

            self.assertEqual(code, 1)
            error = json.loads(stdout.getvalue())["error"]
            self.assertEqual(error["code"], "ERR_ORRO_UNTRACKED_EVIDENCE_NOT_IN_ISOLATED_WORKTREE")
