...
from witnessd.cli.status import build_status, render_status_text, write_status_document
...
class OrroStatusTests(unittest.TestCase):
    def test_write_status_document_projects_the_evidence_view(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(path, repo / ".orro" / "STATUS.md")
            self.assertIn("AUTO-GENERATED by orro", document)
            self.assertIn("Do not hand-edit", document)
            self.assertIn("Status item", document)
    ...
    def test_status_write_failure_is_reported_without_changing_status_result(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("witnessd.cli.status.write_status_document", side_effect=OSError("read-only")):
                with redirect_stdout(io.StringIO()) as stdout, redirect_stderr(io.StringIO()) as stderr:
                    result = main(["orro", "status", "--repo", str(repo), "--write"])
            self.assertEqual(result, 0)
            self.assertIn("ORRO status", stdout.getvalue())
            self.assertIn("STATUS.md", stderr.getvalue())

    def test_boundary_refresh_skips_without_roadmap_or_with_opt_out(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            repo = root / "repo"
            repo.mkdir()
