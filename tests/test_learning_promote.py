...
from witnessd.pause import append_user_pause
...
class TestLearningPromote(unittest.TestCase):
    ...
    def test_promote_delta_refused_while_paused(self):
        with tempfile.TemporaryDirectory() as d:
            priv, pub = gen_operator_keypair(d)
            log = EventLog(os.path.join(d, "runlog.jsonl"))
            cap = {
            ...
            self.assertFalse(result["promoted"])
            self.assertIn("ERR_WITNESSD_PAUSED", result["errors"])
            self.assertFalse(os.path.exists(os.path.join(d, "learning-delta.json")))
