...
class TestW10LiveScript(unittest.TestCase):
    ...
    def test_fake_codex_rehearsal_emits_evidence_and_no_private_key(self):
        ...
        with (
            ...
            self.assertNotEqual(command_log["command"], receipt["invocation"])
            self.assertIn(str(sandbox), command_log["command"])
            self.assertNotIn(str(sandbox), receipt["invocation"])
            run_intent = json.loads((evidence / "run-intent.json").read_text())
            self.assertEqual(run_intent["intent"]["capture_profile"], "redacted")
            redaction_manifest = json.loads(
                (evidence / "redaction-manifest.json").read_text()
            )
