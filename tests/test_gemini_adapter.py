...
class TestGeminiAdapter(unittest.TestCase):
    def test_review_lane_is_headless_and_read_only(self):
        with tempfile.TemporaryDirectory() as sandbox, tempfile.TemporaryDirectory() as bindir:
            ...
            self.assertEqual(res.invocation[res.invocation.index("--approval-mode") + 1], "plan")
            self.assertIn("--output-format", res.invocation)
            self.assertEqual(res.invocation[res.invocation.index("--output-format") + 1], "stream-json")
            self.assertIn("-p", res.invocation)
            self.assertEqual(res.test_output, {"status": "not-run"})
            self.assertEqual(validate_runner_receipt(res.to_runner_receipt(
                arm="direct",
                task_id="review-t",
                worktree=sandbox,
