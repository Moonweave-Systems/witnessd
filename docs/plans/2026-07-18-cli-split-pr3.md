...
- [ ] Rewire: `run` (2102) and `proofrun` (2124) `set_defaults` → `_cli_handler("run", "_cmd_run")`.
- [ ] Test migrations (same commit): `tests/test_orro_workflow.py` — 7 import sites of `_role_lane_plan_team_specs` (1267, 1294, 1340, 1396, 1528, 1556, 1583) → `from witnessd.cli.run import _role_lane_plan_team_specs`; `tests/test_w18_dx.py:121` `witnessd_cli._cmd_run_goal` (inspect.getsource) → `from witnessd.cli import run` + `run._cmd_run_goal` (file already imports `runtime_ops` the same way).
- [ ] `$RUN discover -s tests` → 821 OK. Commit: `refactor(cli): move run/proofrun execution cluster to witnessd.cli.run`
...
- [ ] `verify.py` module scope: stdlib per grep (`subprocess` is required — `_cmd_orro_doctor` :1775 uses `subprocess.run`) + `from witnessd.cli._output import _emit_orro_error, _depone_subprocess_env, _run_depone_json, _hash_file` (grep exact subset) + `from witnessd.cli.run import _reference_adapter_markers, _stamp_reference_adapter_artifact` (cross-cluster callees, `_cmd_proofcheck` :1473/:1475). Existing in-function team_go import moves verbatim.
- [ ] Rewire: `proofcheck` (2179), `advisory-provenance-check` (2188), `handoff` (2199), `orro-doctor` (2227), `engine-lock` (2237) → `_cli_handler("verify", ...)`.
- [ ] Test migrations (same commit): `tests/test_orro_public_flow.py:2334, 2377, 2407` `patch("witnessd.__main__._run_depone_json")` → `patch("witnessd.cli.verify._run_depone_json")` (usage site is `_cmd_proofcheck`); `tests/test_orro_workstyle.py:102` `patch("witnessd.__main__.subprocess.run")` → `patch("witnessd.cli.verify.subprocess.run")` (drives `orro doctor`).
- [ ] `$RUN discover -s tests` → 821 OK. Commit: `refactor(cli): move proofcheck/handoff/doctor/engine-lock to witnessd.cli.verify`
