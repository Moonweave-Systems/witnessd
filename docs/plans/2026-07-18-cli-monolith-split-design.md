...
- `witnessd/__main__.py` ends as: parser wiring + argv normalizers (`_normalize_*`) + `ORRO_COMMAND_MAP`/`ORRO_COMMANDS` + lazy dispatch + `main()` (~700 lines).
...
| 3 | The shared-helper nexus: run/run-goal/proofrun, proofcheck/handoff/doctor-orro/engine-lock, plan/flowplan; team-spec builders (`_role_lane_plan_team_specs`, `_attach_role_capability_team_fields`, `_default_team_lane_command`, `_lane_packet_to_run_team_spec`) → `cli/team_specs.py`; `_build_parser` decomposition; final `__main__` slim-down |
