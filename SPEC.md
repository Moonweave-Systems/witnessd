...
3. **key 회전 정책**: operator Ed25519 key의 회전 주기, 회전 시 과거에 서명된 evidence의 재검증 가능성(구 공개키를 archive로 보관해 과거 evidence를 계속 재도출 가능하게 할지, 아니면 회전 시점 이전 evidence는 별도 archive verdict로 격하할지)이 미정이다. 트리거: 첫 프로덕션 팀 배포 전에 반드시 확정 — key 손상(compromise) 대응 런북이 없는 상태로 배포하지 않는다.
...
| operator private key 유출 시 임의 evidence 위조 가능 | key는 evidence bundle 밖에만 보관(런타임 프로세스 메모리/HSM/OS keychain), verify 경로에 절대 로드하지 않음을 코드 리뷰 게이트로 강제. key 회전 런북(§8.2-3) 확정 전에는 프로덕션 배포 금지. |
...
- **keyless 서명 축 (Sigstore Fulcio keyless + Rekor transparency log)**: W1~W5 완료 후 별도 웨이브(W6 후보)로 착수. 전제조건 — operator-key 서명 축(현재 스코프, assurance 등급 아님)이 최소 1개 프로덕션 팀 배포에서 key 회전 런북과 함께 운영 검증을 마친 뒤에만 keyless로 확장한다. keyless 도입 시에도 operator-key 경로를 deprecate하지 않고 병행 지원(두 서명 모델 모두 Depone이 재도출 가능해야 함).
