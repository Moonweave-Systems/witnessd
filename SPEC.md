...
### 3.10 keyless 서명 축 (Rekor/Fulcio) W6a/W6b 경계

> **Decision (재검토 가능):** external-team-pilot production gate는 open이고 5/5 증거가 recorded지만, Sigstore Fulcio keyless + Rekor transparency log(keyless 서명 축)는 W6a에서 여전히 fail-closed다. 이유는 gate가 아니라 신뢰 가능한 live Fulcio/Rekor/OIDC verifier가 아직 없기 때문이다. **Rationale (3가지):**
> 1. **Depone은 keyless 메타데이터를 아직 신뢰하지 않는다.** W6a의 keyless linter는 fixture integrity만 검사하며 `signature_verified=false`, `trusts_external_signature=false`, `keyless_identity=false`, `transparency_logged=false`를 유지한다. `ingest_signed_evidence_bundle`은 operator-key-only이고 keyless ingest는 없다.
> 2. **stdlib/openssl-only 제약과 air-gapped wedge.** witnessd runtime은 stdlib+openssl CLI만 허용하고, W6a는 Fulcio(OIDC 발급)·Rekor(공개 transparency log) 네트워크 접근이나 Sigstore 의존성을 추가하지 않는다. operator-key 서명은 오프라인·no-network로 완결되어 air-gapped 소비와 맞는다.
> 3. **W6b로 분리할 검증기 설계가 필요하다.** signing 단계는 어댑터에서 교체 가능하게 설계하되(§3.5), live keyless는 Fulcio 인증서 체인 검증 + Rekor inclusion proof + dependency/network 승인 설계를 Depone verify 경로에 추가하는 별도 wave에서 다룬다. 그때까지 witnessd는 **절대 keyless/transparency-logged 시맨틱을 주장하지 않는다.**
...
- **lane별 필드:** `lane_id`(중복 금지 → `ERR_TEAM_LEDGER_LANE_ID_DUPLICATE`), optional `lane_intent ∈ {implementation, verification-only}`(미선언은 implementation; declared verification-only는 아래 empty-`touched_files` 예외 적용), `verification_state ∈ {pass, blocked}`(`VALID_LANE_VERIFICATION_STATES`), `env_kind ∈ {local, container, cloud}`(`VALID_ENV_KINDS`), adapter kind(`VALID_ADAPTER_KINDS`), `evidence_dir`, `touched_files`(undeclared/implementation passed lane은 ≥1개 → 없으면 `ERR_TEAM_LEDGER_TOUCHED_FILES_REQUIRED`), worktree receipt(§4.7), `evidence_next_verdict`(아래).
...
- **declared verification-only lane 처리(구조적 에지케이스, §6 참조).** plan이 실행 전에 `lane_intent:"verification-only"`를 선언한 lane은 파일을 바꾸지 않아도 team-ledger `lanes`에 `verification_state:"pass"`, empty `touched_files`, worktree receipt `changed_files:[]`로 fan-in한다. Depone의 falsifiable gate `ERR_TEAM_LEDGER_VERIFICATION_LANE_MUTATED`는 이 선언 lane이 non-empty `touched_files` 또는 `changed_files`를 보고하면 state와 무관하게 reject한다. 미선언 lane(= implementation)은 기존 `touched_files ≥ 1` 요구를 유지한다. 종전 clause의 premise인 "passed lane은 `touched_files ≥ 1`을 요구하므로"는 Depone v0.2.2에서 verification-only 예외 도입으로 repeal되었으므로 본 확정 clause를 이에 맞게 amend한다. `lane_intent`는 observed diff/receipt에서 post-hoc derive하지 않고 plan/config의 사전 선언만 전달한다.
...
3. **key 회전 정책**: operator Ed25519 key는 90일 주기 또는 compromise 의심 즉시 회전한다. 과거 evidence는 retired public key archive로 계속 재검증 가능하게 유지하고, compromised key는 compromise 시점 이후 evidence를 신뢰 목록에서 제외한다. 첫 프로덕션 팀 배포 전 hard gate는 `docs/ops/operator-key-rotation.md`를 실제로 실행하고 `scripts/revalidate_key_rotation.py`가 archive/canary evidence를 통과하는 것이다. 여기서 프로덕션 팀 배포는 `external-team-pilot`으로 고정한다: local dogfood, hand-authored fixture, CI-only run은 제외하며, named deployment record·rotated archive·current-key canary·Depone verification transcript·operator review가 모두 기록되어야 한다. 현재 committed archive는 `external-team-pilot` 5증거가 recorded이고 `production_gate.status="open"`인 상태로 `scripts/revalidate_key_rotation.py`를 통과한다. keyless(Fulcio/Rekor)는 gate가 아니라 live Fulcio/Rekor verifier 미구현 때문에 W6b 전까지 emit/ingest/trust 금지다.
...
- **keyless 서명 축 (Sigstore Fulcio keyless + Rekor transparency log)**: W6a는 non-trusting keyless fixture linter와 witnessd fail-closed signing profile seam만 추가한다. 전제조건이던 `external-team-pilot` production gate는 이미 open이며 `fixtures/key-rotation/operator-key-archive.json`의 `production_gate.required_evidence` 5종이 모두 `recorded`이고 `scripts/revalidate_key_rotation.py`가 이를 통과한다. W6b/live keyless는 Fulcio/Rekor/OIDC 의존성·네트워크 검증 설계가 승인된 뒤에만 착수한다. keyless 도입 시에도 operator-key 경로를 deprecate하지 않고 병행 지원(두 서명 모델 모두 Depone이 재도출 가능해야 함).
