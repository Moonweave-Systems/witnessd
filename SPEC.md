...
- **keyless 서명 축 (Sigstore Fulcio keyless + Rekor transparency log)**: W1~W5 완료 후 별도 웨이브(W6 후보)로 착수. 전제조건 — operator-key 서명 축(현재 스코프, assurance 등급 아님)이 최소 1개 `external-team-pilot` 배포에서 key 회전 런북과 archive revalidation과 함께 운영 검증을 마친 뒤에만 keyless로 확장한다. 통과 조건은 `fixtures/key-rotation/operator-key-archive.json`의 `production_gate.required_evidence` 5종이 모두 `recorded`이고 각 항목에 repo-relative artifact path와 stable artifact hash가 있으며, `scripts/revalidate_key_rotation.py`가 이를 통과하는 것이다. keyless 도입 시에도 operator-key 경로를 deprecate하지 않고 병행 지원(두 서명 모델 모두 Depone이 재도출 가능해야 함).
