...
A2는 demonstration-only. 시장 조사(2026-07-02): 검증 레이어는 붐비기 시작(OVERT/EQTY/MS/Agent
...
- **W12 Real A2** — 이 호스트에 uid 격리 셋업(sudo 가능 실측 확인됨), demonstration 딱지 제거,
...
- **D12-2. demonstration 딱지 제거는 증거로만.** 진짜 격리 run이 `A2-isolated-observed`를 방출하고
  Depone strict assert가 통과할 때만 `fixtures/w1/A2-DEMONSTRATION.md`를 제거한다(문서 먼저 제거 금지).
...
4. README/conformance 갱신: "실제 에이전트 E2E 실증" 추가, A2 demonstration 문구 제거(W12 후),
   과대주장 금지 유지.
