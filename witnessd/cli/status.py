...
def _cmd_orro_status(args: argparse.Namespace) -> int:
    ...
    if args.write and (repo / ".orro" / "roadmap.json").is_file():
        try:
            ...
            print(f"warning: could not write .orro/STATUS.md: {exc}", file=sys.stderr)
    if args.json:
        print(json.dumps(payload, sort_keys=True))
    else:
...
def write_status_document(repo: Path, home: Path) -> Path:
    ...
    except Exception:  # noqa: BLE001 - source checkout has no installed metadata
        version = "2.28.0"
    timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    lines = [
        f"<!-- AUTO-GENERATED by orro {version} from run evidence at {timestamp}. Do not hand-edit; the tool never reads this file. Regenerate: orro status --write -->",
        "",
        "# ORRO status",
        "",
        "This file is a write-only projection of roadmap and run evidence; truth stays in evidence.",
        "",
        "## Roadmap",
    ]
    ...
    lines.extend([
        ...
        (
            ...
            f"managed={workspace['managed_worktree_count']}, eligible={workspace['eligible_worktree_count']}, "
            f"external={workspace['external_worktree_count']}, size={workspace['worktree_bytes']} bytes, "
...
def refresh_status_document(args: argparse.Namespace) -> None:
    if os.environ.get("ORRO_NO_STATUS_DOC") == "1":
        return
    repo_value = getattr(args, "repo", None) or getattr(args, "root", None)
    repo = Path(repo_value).resolve(strict=False) if repo_value else Path.cwd().resolve(strict=False)
    if not (repo / ".orro" / "roadmap.json").is_file():
        return
    home = resolve_home(getattr(args, "home", None), repo)
    try:
        write_status_document(repo, home)
    except Exception as exc:  # noqa: BLE001 - status projection never changes command result
        print(f"warning: could not refresh .orro/STATUS.md: {exc}", file=sys.stderr)
            ...
            off_plan.append(
                {
                    ...
                    "state": run["state"],
                    **(
                        {"ship_command": run["ship_command"]}
                        if run.get("ship_ready")
                        else {}
                    ),
                }
            )
...
def _status_run(run_dir: Path, *, home: Path) -> dict[str, Any]:
    ...
    if decision.get("ship_ready"):
        result["ship_ready"] = True
        result["ship_command"] = decision["ship_command"]
    return result
