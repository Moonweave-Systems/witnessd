...
def write_status_document(repo: Path, home: Path) -> Path:
    ...
    header = (
        f"<!-- AUTO-GENERATED by orro {version} from run evidence at {timestamp}. "
        "Do not hand-edit; the tool never reads this file. Regenerate: "
        "orro status --write -->"
    )
    lines = [
        header,
    ...
    if path.is_file():
        ...
        _, separator, existing_body = existing.partition("\n")
        if separator and existing_body == body:
            return path
